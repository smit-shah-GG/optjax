...
class JaxCryptoEnv(environment.Environment):
    def __init__(self):
        super().__init__()
        self.price_array, self.tech_array_total = DiskDataLoader(dataset_name='dataset').load_dataset()
        # Fix: Calculate observation_dim during initialization
        self.lookback_window_len = EnvParams().lookback_window_len
        self.observation_dim = (self.tech_array_total.shape[1] + 2) * self.lookback_window_len
    ...
    def step_env(
        ...
        def handle_action_1(carry):
            ...
            new_average_price_long = jnp.where(
                buy_amount > 0,
                (state.position_value_long + buy_amount * price_ask) / (state.coins_long + buy_amount),
        ...
        def handle_action_2(carry):
            ...
            short_amount = jnp.where(
                (-state.coins_short >= 0) & (state.available_balance > params.order_size),
                params.order_size / price_ask,
                0.0
            )
        ...
        def handle_action_3(carry):
            ...
            realized_pnl_long = state.coins_long * (price_bid - state.average_price_long)
            realized_pnl_short = -state.coins_short * (state.average_price_short - price_ask)
        ...
        fee_to_close = jnp.abs(position_value_short + position_value_long) * params.close_fee

        margin_short = initial_margin_short + params.maintenance_margin_percentage * position_value_short
        margin_long = initial_margin_long + params.maintenance_margin_percentage * position_value_long

        total_margin = margin_short + margin_long + fee_to_close
        available_balance = jnp.maximum(new_state.wallet_balance - total_margin, 0)

        unrealized_pnl_short = -new_state.coins_short * (new_state.average_price_short - price_ask)
        unrealized_pnl_long = new_state.coins_long * (price_bid - new_state.average_price_long)

        next_equity = new_state.wallet_balance + unrealized_pnl_short + unrealized_pnl_long
        ...
        reward = (unrealized_pnl_short + unrealized_pnl_long -
                 (new_state.unrealized_pnl_short + new_state.unrealized_pnl_long)) / params.initial_capital
    ...
    def reset_env(
        ...
        initial_state = EnvState(
            ...
            state_queue=jnp.zeros((params.lookback_window_len, self.tech_array_total.shape[1] + 2)),
            reset_queue=jnp.zeros((params.lookback_window_len * 4, self.tech_array_total.shape[1] + 2)),
            liquidation=False,
    ...
    def _get_observation_reset(
        ...
        def scan_fn(carry, time_idx):
            queue = carry
            current_time = state.time_absolute - params.lookback_window_len * 4 + time_idx

            input_array = tech_array[current_time]
            current_observation = jnp.concatenate([
                input_array[:2],
                jnp.array([state.available_balance, state.unrealized_pnl_short + state.unrealized_pnl_long]),
                input_array[2:]
            ])

            return jnp.roll(queue, -1, axis=0).at[-1].set(current_observation), None

        init_queue = jnp.zeros((params.lookback_window_len * 4, tech_array.shape[1] + 2))
        final_queue, _ = lax.scan(
            scan_fn,
            init_queue,
            jnp.arange(params.lookback_window_len * 4)
        )
...
# Fix 1: Pure Scaler Implementation
class Scaler:
    def __init__(self, min_quantile: float = 0.5, max_quantile: float = 99.5, scale_coef: float = 1000.):
        self.min_quantile = min_quantile
        self.max_quantile = max_quantile
        self.scale_coef = scale_coef

    @partial(jax.jit, static_argnums=(0,))
    def reset(self, state_array: chex.Array, reset_array: chex.Array) -> Tuple[chex.Array, ScalingState]:
        # Calculate scaling parameters purely
        min_values = jax.vmap(lambda x: jnp.nanpercentile(x, self.min_quantile))(reset_array.T)
        max_values = jax.vmap(lambda x: jnp.nanpercentile(x, self.max_quantile))(reset_array.T)

        # Create immutable scaling state
        ...
        scaled_array = self._scale(state_array, scaling_state)
        return scaled_array, scaling_state

    @partial(jax.jit, static_argnums=(0,))
    def step(self, state_array: chex.Array, scaling_state: ScalingState) -> chex.Array:
        return self._scale(state_array, scaling_state)

    def _scale(self, array: chex.Array, scaling_state: ScalingState) -> chex.Array:
        scaled = (array - scaling_state.min_values) / (
            jnp.maximum(scaling_state.max_values - scaling_state.min_values, 1e-8)
        )
        return jnp.clip(scaled, 0, 1) * scaling_state.scale_coef

# Fix 2: Pure Statistics Recorder
@dataclass(frozen=True)
class StatisticsState:
    actions: chex.Array
    rewards: chex.Array
    rewards_realized_pnl_short: chex.Array
    rewards_realized_pnl_long: chex.Array
    unrealized_pnl_short: chex.Array
    unrealized_pnl_long: chex.Array
    margin_short: chex.Array
    margin_long: chex.Array
    num_steps: chex.Array
    coins_short: chex.Array
    coins_long: chex.Array
    equity: chex.Array
    wallet_balance: chex.Array

class StatisticsRecorder:
    @partial(jax.jit, static_argnums=(0,))
    def init_state(self) -> StatisticsState:
        return StatisticsState(
            **{key: jnp.array([]) for key in [
                'actions', 'rewards', 'rewards_realized_pnl_short',
                'rewards_realized_pnl_long', 'unrealized_pnl_short',
                'unrealized_pnl_long', 'margin_short', 'margin_long',
                'num_steps', 'coins_short', 'coins_long', 'equity',
                'wallet_balance'
            ]}
        )

    @partial(jax.jit, static_argnums=(0,))
    def update(self, state: StatisticsState, stats_dict: Dict[str, Union[float, int, bool]]) -> StatisticsState:
        return StatisticsState(
            **{
                key: jnp.append(getattr(state, key), stats_dict.get(key, 0.0))
...
class DiskDataLoader:
    def __init__(self, dataset_name: str = 'dataset'):
        ...
        self.price_shape = (25000, 1)
        self.tech_shape = (25000, 30)
    ...
    def load_dataset(self) -> Tuple[chex.Array, chex.Array]:
        ...
        return jax.pure_callback(
            load_data,
            (jnp.ones(self.price_shape), jnp.ones(self.tech_shape))
        )

# Helper functions
@jax.jit
def create_env() -> Tuple[JaxCryptoEnv, EnvParams]:
    """Create environment instance with default parameters."""
    env = JaxCryptoEnv()
...
def run_episode(
    ...
    def episode_step(carry, _):
        ...
        # Step environment
        obs, next_state, reward, done, info = env.step_env(step_key, state, action, params)
    ...
    final_carry, episode_history = lax.scan(
        episode_step,
        (init_state, rng, 0.),
        jnp.arange(params.episode_max_len)
    )
